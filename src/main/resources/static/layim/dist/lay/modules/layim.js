...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-history').on('contextmenu', 'li', function(e){
      ...
      //好友右键菜单
      layimMain.find('.layim-list-friend').on('contextmenu', ".layui-layim-list li[data-index='3']", function (e) {
          var othis = $(this);
          var id = othis.attr('class');
          var html = '<ul  data-id="' + id + '" >';
          html += '<li layim-event="friendMenu_delete"><i class="layui-icon" >&#xe640;</i>' + '删除好友</li></ul>';
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,chatLog: function(othis){
      ...
      //删除好友右键菜单操作
      ,friendMenu_delete: function(othis){
        layer.closeAll('tips');

         var id = othis.parent().data("id");
         var userId = id.substr(12).trim();
    ...
    ,menuHistory: function(othis, e){
      ...
      if(type === 'one'){
        ...
        //重写移除代码
        var flag = parent.data('index');
        hisElem.find("li[data-index = '"+flag+"']").remove();
